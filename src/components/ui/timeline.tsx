    ...
    >
      <div className="max-w-7xl mx-auto py-20 px-4 md:px-8 lg:px-10">
        <h2 className="text-3xl md:text-4xl mb-4 text-white max-w-4xl font-bold">
          <span className="text-5xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-white to-blue-300">
          Our Core Pillars
          </span>
        </h2>
        <p className="text-gray-300 text-sm md:text-2xl max-w-3xl">
          The foundational principles that drive everything we build and deliver.
        </p>
      </div>
        ...
        {data.map((item, index) => (
          ...
          >
            <div className="sticky flex flex-col md:flex-row z-40 items-center top-40 self-start max-w-xs lg:max-w-sm md:w-full"> 
              <div className="h-10 absolute left-3 md:left-3 w-10 rounded-full bg-gray-900/80 backdrop-blur-sm border border-gray-700 flex items-center justify-center">
            ...
            <div className="relative pl-20 pr-4 md:pl-4 w-full">
              <h3 className="md:hidden block text-1xl mb-4 text-left font-bold text-blue-400">
                {item.title}
              </h3>
              {item.content}
            </div>
